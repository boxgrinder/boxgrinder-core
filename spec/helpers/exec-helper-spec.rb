  ...
  describe ExecHelper do
    ...
    if RUBY_PLATFORM =~ /java/
      it "should fail when command exit status != 0 and is on MRI" do
        ...
        Process.should_receive(:waitpid2).with(1234).and_return([1234, OpenCascade[:exitstatus => 1]])
    ...
    it "should execute the command" do
      ...
      @stderr.should_receive(:each)
      Process.should_receive(:getpgid)
      Process.should_receive(:waitpid2).with(1234).and_return([1234, OpenCascade[:exitstatus => 0]])
